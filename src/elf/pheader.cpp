...
class Elf;
inline auto
execution_view_inner(const auto* self, ranges::contiguous_range auto const& file)
{
...
std::ostream&
operator<<(std::ostream& os, const p_type_t& p_type)
{
...
struct ProgramHeader<X32>
{
  ...
  template<ranges::contiguous_range F,
           ranges::view T = decltype(execution_view_inner(std::declval<const ProgramHeader<X32>*>(),
...
struct ProgramHeader<X64>
{
  ...
  template<ranges::contiguous_range F,
           ranges::view T = decltype(execution_view_inner(std::declval<const ProgramHeader<X64>*>(),
                                                          std::declval<ranges::ref_view<const F>&>()))>
    requires ranges::input_range<T>
  auto ExecutionView(const Elf<F>& file) const -> T
  {
...
template<Width W>
std::ostream&
operator<<(std::ostream& os, const ProgramHeader<W>& pheader)
{
  if constexpr (W == X32) {
